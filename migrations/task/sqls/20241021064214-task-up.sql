...
select name as 專長名稱 ,count(cls.coach_id) as coach_total from "SKILL" s 
inner join "COACH_LINK_SKILL" cls on cls.skill_id = s.id 
group by s."name" 
...
select package.name as 方案名稱,sum(buy.purchased_credits) as 銷售數量 from "CREDIT_PURCHASE" buy
inner join "CREDIT_PACKAGE" package on package.id = buy.credit_package_id 
where buy.purchase_at  between '2024-12-01' and '2024-12-30'
group by package.name;

-- 6-4. 查詢：計算 11 月份總營收（使用 purchase_at 欄位統計）
-- 顯示須包含以下欄位： 總營收
select sum(price_paid) as 總營收 from "CREDIT_PURCHASE" buy
...
where created_at between '2024-12-01' and '2024-12-31' and status = '即將授課';
